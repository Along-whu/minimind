...
def aply_lora(model, rank=8):
    for name, module in model.named_modules():
        if (
            isinstance(module, nn.Linear())
            and module.weight.shape[0] == module.weight.shape[1]
        ):
            lora = Lora(module.weight.shape[0], module.weight.shape[1], rank).to(
                model.device
            )
            setattr(module, "lora", lora)
            original_forward = module.forward

            def forward_with_lora(x, layer1=original_forward, layer2=lora.forward):
