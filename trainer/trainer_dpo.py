...
def dpo_loss(ref_log_probs, policy_log_probs, mask, beta):
    ...
    # mask shape: [2B, L] 只有回答部分是1，其余为0
    seq_len = mask.sum(1, keepdim=True).clamp_min(1e-8)
    
    # mask部分的概率清0，求和，做平均
    # [2B, L] -> [2B]
    ref_log_probs = (ref_log_probs * mask).sum(1) / seq_len.squeeze()
    policy_log_probs = (policy_log_probs * mask).sum(1) / seq_len.squeeze()
...
def train_epoch(epoch, loader, iters, ref_model, lm_config, start_step=0, wandb=None, beta=0.1):
    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        # chosen和rejected的输入tokens
        x_chosen = batch['x_chosen'].to(args.device)
        x_rejected = batch['x_rejected'].to(args.device)
        
        # chosen和rejected的标签labels
        ...
        with autocast_ctx:
            ...
            # 取出标签token的log_probs
            # [2B, L]
            ref_log_probs = logits_to_probs(ref_logits, y)
            
            outputs = model(x)
            logits = outputs.logits
            # [2B, L]
            policy_log_probs = logits_to_probs(logits, y)
            
            # 计算DPO loss
            dpo_loss = dpo_loss(ref_log_probs, policy_log_probs, mask, beta=beta)
            
            loss = dpo_loss + outputs.aux_loss
