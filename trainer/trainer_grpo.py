    ...
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        with autocast_ctx:
            ...
            outputs = model_for_gen.generate(
                **prompt_inputs, max_new_tokens=args.max_gen_len, do_sample=True,
                temperature=0.8,
                num_return_sequences=args.num_generations, # G条回答
                pad_token_id=tokenizer.pad_token_id
            ) # [B * G, P + R]
            
            completion_ids = outputs[:, prompt_inputs.size(1):] # 只取回答的部分 [B * G, R]
            
            def get_per_token_logps(mdl, input_ids, n_keep):
                input_ids = input_ids.detach().clone() if input_ids.is_inference() else input_ids
                
                # 由于我们只取R的logits，所以要取最后R + 1个，最后一个不用，因为没有label对比
                # [P0, P1, R0, R1, R2]
                # [L0, L1, L2, L3, L4]
                #         [L1, L2, L3]
                logits = mdl(input_ids, logits_to_keep=n_keep + 1).logits[:, :-1, :] # [B*G, R]
                
                per_token_logps = []
                for logits_row, ids_row in zip(logits, input_ids[:, -n_keep:]):
                    ids_row = ids_row.detach().clone() if ids_row.is_inference() else ids_row
                    
                    per_token_logps.append(torch.gather(logits_row, dim=-1, index=ids_row.unsqueeze(1)).squeeze(1))
            ...
            mean_r = grouped_rewards.mean(1).repeat_interleave(args.num_generations) # [B*G]
            std_r = grouped_rewards.std(1).repeat_interleave(args.num_generations) # [B*G]
            
            # 计算标准化优势
            advantages = torch.clamp((rewards - mean_r) / (std_r + 1e-4), -10, 10) # [B * G]
            advantages = (advantages - advantages.mean()) / (advantages.std() + 1e-8) # [B*G]
            
            # 计算每一个句子的eos的位置，如果没有，就设置为R
            is_eos = completion_ids == tokenizer.eos_id # [B*G, R]
            eos_idx = torch.full((is_eos.size(0),), is_eos.size(1), dtype=torch.long, device=args.device) # [B*G]
            eos_idx[is_eos.any(dim=1)] = is_eos.int().argmax(dim=1)[is_eos.any(dim=1)] # [B*G]
            completion_mask = (torch.arange(is_eos.size(1), device=args.device).expand(is_eos.size(0), -1) <= eos_idx.unsqueeze(1)).int() # [B*G, R]
            
            # logp_ref - logp_pi
            kl_div = ref_per_token_logps - per_token_logps # [B*G, R]
            
            per_token_kl = torch.exp(kl_div) - kl_div - 1 # 近似于 (kl_div)^2 / 2  [B*G, R]
            
            # GRPO Loss = -(pi / ref * A - beta * kl) [B*G, R]
            per_token_loss = -(torch.exp(per_token_logps - ref_per_token_logps.detach()) * advantages.unsqueeze(1) - args.beta * per_token_kl)
