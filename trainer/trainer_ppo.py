...
def ppo_train_epoch(
    ...
    for step, batch in enumerate(loader):
        ...
        # 2. Rollout/Generate actor_model make experience
        with torch.no_grad():
            model_for_gen = (
                actor_model.module
        ...
        # 4. Critic & Advantage critic_model 得到values & 计算Advantages
        full_mask = (gen_out != tokenizer.pad_token_id).long()  # [B, P+R]
        with torch.no_grad():
            values_seq = critic_model(
                input_ids=gen_out, attention_mask=full_mask
            )  # [B, P+R]
            last_indices = (full_mask * torch.arange(full_mask.size(1), device=gen_out.device)).argsmax(dim=1) # 最后一个token的索引
            values = values_seq[torch.arange(values_seq.size(0), device=values_seq.device), last_indices] # [B] 最后一个token的value
            
            advantages = rewards - values.detach() # [B] A = R - V
        
        # 5. 计算actor、olg、ref策略的对数概率
        with autocast_ctx:
        ...
        logp_token = F.log_softmax(logits[:, :-1], dim=-1).gather(dim=2, index=labels.unsqueeze(-1)).squeeze(-1) # [B, P+R-1]
        seq_len = gen_out.size(1) - 1
        resp_mask = torch.arange(seq_len, device=gen_out.device).unsqueeze(0) >= prompt_length - 1 # mask掉promt
        final_mask = resp_mask & (~labels.eq(tokenizer.eos_token_id)) # [B, P+R-1]
        actor_logp = (logp_token * final_mask).sum(1) # [B]
        
        with torch.no_grad():
            # 5.2 old actor logp
            old_logits = old_actor_model(input_ids=gen_out, attention_mask=full_mask).logits # [B, P+R, V]
            old_logp_token = F.log_softmax(old_logits[:, :-1], dim=-1).gather(dim=2, index=labels.unsqueeze(-1)).squeeze(-1)
            old_logp = (old_logp_token * final_mask).sum(1) # [B]
            
            # 5.3 ref logp            
            ref_logits = ref_model(input_ids=gen_out, attention_mask=full_mask).logits # [B, P+R, V]
            ref_logp_token = F.log_softmax(ref_logits[:, :-1], dim=-1).gather(dim=2, index=labels.unsqueeze(-1)).squeeze(-1)
            ref_logp = (ref_logp_token * final_mask).sum(1) # [B]
            
        # 6. PPO
        kl = (actor_logp - old_logp).mean() # 
        kl_ref = (actor_logp - ref_logp).mean() # KL散度=log(actor_p / ref_p)
        
        ratio = torch.exp(actor_logp - old_logp).mean() # ratio = new / old
        ...
        del logits, labels, logp_token, final_mask, actor_logp, old_logits, old_logp, ref_logits, ref_logp
